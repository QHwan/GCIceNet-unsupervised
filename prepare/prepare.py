...
parser.add_argument('--inv_a_hat', type=str, help='a inv hat file of sparse matrix')
...
for i in tqdm(range(n_frame)):
    ...
    idx_sorted_ngbs_dist_ow_mat = np.copy(idx_sorted_dist_ow_mat[:,1:7])
    ...
    for j in range(n_ow):
        for k in range(n_ow):
            if j == k:
                inv_a_hat[j, k] = 2/(np.sum(buf[j])+2)
            elif buf[j, k] == 0:
                pass
            else:
                inv_a_hat[j, k] = -buf[j, k] / np.sqrt((np.sum(buf[j])+2) * (np.sum(buf[k])+2))

    adj = sp.coo_matrix(adj)
    a_hat = sp.coo_matrix(a_hat)
    inv_a_hat = sp.coo_matrix(inv_a_hat)

    o_adj.append(adj)
    o_a_hat.append(a_hat)
    o_inv_a_hat.append(inv_a_hat)
...
with open(args.inv_a_hat, 'wb') as f:
    pickle.dump(o_inv_a_hat, f)
